...
class BaseOpenAIClient(LLMClient):
    ...
    def __init__(
        ...
        # Disable reasoning for models that don't support extended thinking
        # Only o1-series models support reasoning.effort parameter
        if config.model and not config.model.startswith('o1'):
            self.reasoning = None
        else:
    ...
    def _handle_structured_response(self, response: Any) -> dict[str, Any]:
        ...
        if hasattr(response, 'choices') and response.choices:
            ...
            if content:
                ...
                raise Exception(f'Empty response content from LLM: {response.model_dump()}')
        # Handle responses.parse() API response format (fallback)
